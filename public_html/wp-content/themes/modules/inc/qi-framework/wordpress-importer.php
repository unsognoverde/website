...
class WP_Import extends WP_Importer {
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'wordpress-importer' ) . '</strong><br />';
			printf( wp_kses_post( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'wordpress-importer' ) ), esc_html( $file['file'] ) );
			echo '</p>';
			return false;
		}
...
<?php if ( ! empty( $this->authors ) ) : ?>
	...
	<p><?php wp_kses_post( _e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'wordpress-importer' ) ); ?></p>
...
function wordpress_importer_init() {
	...
	register_importer( 'wordpress', 'WordPress', wp_kses_post( __('Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', 'wordpress-importer') ), array( $GLOBALS['wp_import'], 'dispatch' ) );
}
