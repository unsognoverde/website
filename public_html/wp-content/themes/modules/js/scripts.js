...
jQuery(document).ready(function() {
	...
	if ( $masPortfItem.length ) {
		var itemContent = jQuery('.item-content');
		// Adjust faux element to content height for masonry purposes
		jQuery('.faux-element').height(itemContent.height()+60).width(itemContent.width()+60);
	}
		...
		itemContent.animate({ opacity: 0 }, function(){
			...
			jQuery.post(ajaxurl, {
				...
				if ( response != 0 ) {
					...
					// Define Content Height for Portfolio Item (masonry style)
					if ( masPortfThisItem.length ) {
						var itemContentHeader = jQuery('.item-content');
						// Adjust faux element to content height for masonry purposes
						jQuery('.faux-element').height(itemContentHeader.height()+60).width(itemContentHeader.width()+60);
					}
	...
	jQuery('.filter-terms li').click(function() {
		...
		console.log('Looking for elements with class cat' + thisCat.data('id'));
	});
